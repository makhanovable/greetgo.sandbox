...
public interface ClientDao {
  @Update("UPDATE client SET actual=0 WHERE ciaId=#{ciaId}")
  void deleteRowById(@Param("ciaId") long id);

  @Select("SELECT EXISTS (SELECT true FROM client WHERE ciaId=#{ciaId} AND actual=1)")
  boolean selectExistsRowById(@Param("ciaId") long id);
  ...
  ClientDetails selectDetailsById(@Param("ciaId") long id);
  ...
  void insert(@Param("ciaId") long id,
  ...
  @Update("UPDATE client " +
    ...
    "WHERE ciaId=#{ciaId}")
  void update(@Param("ciaId") long id,
              @Param("surname") String surname,
              @Param("name") String name,
              @Param("patronymic") String patronymic,
              @Param("gender") String gender,
